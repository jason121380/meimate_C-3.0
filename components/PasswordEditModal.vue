    ...
    <transition name="fade">
      <div v-if="showModal"
        ...
        <div class="w-full max-w-[768px] max-h-[80vh] bg-white rounded-2xl relative z-[-1] shadow-lg overflow-hidden">
          <!-- 關閉按鈕 -->
          <button @click="closeModal" class="absolute right-6 top-5 text-gray-400 active:text-gray-600 cancel">
            ...
            <form @submit.prevent="handleSubmit(setpassword)">
                ...
                <div class="mb-5">
                  ...
                  <ValidationProvider name="密碼" rules="required|onlyEnNum" v-slot="{ errors }" class="w-full">
                    <input name="密碼" v-model="password" onpaste="return false" id="password1Input" type="password"
                      class="w-full bg-gray-50/80 border border-gray-200 rounded-full px-5 py-3 text-sm text-gray-700 focus:outline-none focus:border-gmb-orange-400 focus:ring-1 focus:ring-gmb-orange-200"
                      placeholder="請輸入新密碼"
              ...
              <div class="border-t border-gray-100 p-5 flex justify-end bg-gray-50/30">
                <button :disabled="isComfirm" type="submit"
                  class="py-2.5 px-8 flex items-center gap-2 text-white text-[15px] font-semibold bg-gmb-orange-500 rounded-full active:bg-gmb-orange-600 shadow-sm"
